...
export default function VerifyPage() {
    ...
    // 🔹 Run verification on a mount
    useEffect(() => {
        const ref = params.get("paymentReference");
        if (ref) verifyPayment(ref);
    }, [params]);

    // ✅ Verify payment and resolve event link
    const verifyPayment = async (paymentReference: string) => {
        try {
            ...
            if (data.success && data.result) {
                ...
                // Fetch a corresponding event link (admin-generated)
                const eventId = tx.product;
                if (eventId) {
                    const docRef = doc(db, "events", eventId);
                    const eventSnap = await getDoc(docRef);
                    if (eventSnap.exists()) {
                        const link = eventSnap.data().eventLink as string | undefined;
                        if (link) setEventLink(link);
                    }
                }
    ...
    const handleSendEmail = async () => {
        ...
        try {
            ...
            console.error(err);
            toast.error("Error sending or downloading ticket.");
        } finally {
